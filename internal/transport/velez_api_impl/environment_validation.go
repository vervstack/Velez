...
// CreateSmerd/ListSmerds/DropSmerd/UpgradeSmerd requests and maps it onto the
// Docker suffix.
//
// proto3 has no `required` keyword, so this - plus its service_api_impl twin -
...
func (impl *Impl) resolveEnvironment(ctx context.Context, environment string) (string, error) {
	suffix, err := impl.vervServices.ResolveEnvironmentSuffix(ctx, environment)
	if err != nil {
