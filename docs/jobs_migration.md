...
**Status: complete — `internal/pipelines` is deleted.** The original 7-pipeline `Pipeliner` migration this doc
tracks finished with all pipelines cut over to live RPCs except `CopyToVolume` (no live caller), and `DropSmerd` —
never part of `Pipeliner` to begin with — scaffolded and cut over too (2026-07-26). What kept the package itself
alive after that was two *non-RPC* callers this table's `ConnectServiceToVpn`/`UpgradeSmerd` rows flag below:
`internal/cluster/autoupgrade/autoupgrade.go` (`UpgradeSmerd`) and `internal/workers/deploy_watcher.go`
(`LaunchSmerd` + `UpgradeSmerd`, plus two raw `Docker()` calls). Both are now migrated — `deploy_watcher.go` onto
`Engine.Enqueue`/`Watch` (same sync-facade shape as every RPC cutover above) and resolved `ContainerRuntime` for
its container operations, `autoupgrade.go` onto `Engine.Enqueue`/`Watch` too (staying implicitly PROD-only, a
pre-existing limitation, not a new one — see its own doc comment). `internal/pipelines` — `pipelines.go`,
`runner.go`, every `do_*.go`, all of `steps/*` — is deleted in full as of this pass. The one thing *not* deleted:
`do_connect_service_to_vpn.go`'s free function `ConnectServiceToVpn` (see its own table row below for why it was
never a simple `Pipeliner` removal) moved verbatim to a new package, `internal/cluster/vpnconnect`, for the same
reason its removal was blocked before — its two node-bootstrap callers need the typed sentinel errors
`Engine.Enqueue` would flatten to strings. See [`docs/roadmap.md`](roadmap.md) for anything else outstanding
across the codebase; this file is kept as the detailed per-pipeline history and as the recipe/checklist for
migrating any future pipeline (still useful as a reference even though `internal/pipelines` itself is gone — the
job-authoring pattern it describes is unchanged).
...
**Update — the blockers below are resolved.** The `ConnectServiceToVpn` and `UpgradeSmerd` rows above describe, as
of when they were written, non-RPC callers that blocked deleting `do_connect_service_to_vpn.go`/
`do_smerd_upgrade.go`. Both blockers are gone: `autoupgrade.go` and `deploy_watcher.go` (the two direct
`Pipeliner.UpgradeSmerd`/`LaunchSmerd` callers) are migrated onto the jobs engine, and `do_smerd_upgrade.go`
itself is deleted along with the rest of `internal/pipelines`. `ConnectServiceToVpn`'s free function survives, but
relocated to `internal/cluster/vpnconnect`, not left in `internal/pipelines` — see the status line at the top of
this file. Left the table rows themselves unedited since they're accurate history of the state at cutover time.
