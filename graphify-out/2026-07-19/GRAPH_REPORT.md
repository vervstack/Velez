# Graph Report - Velez  (2026-07-19)

## Corpus Check
- 573 files · ~259,511 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6448 nodes · 10653 edges · 759 communities (372 shown, 387 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 527 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `da829dcb`
...
- internal/api (ListEnvironments_Request)
...
- internal/clients (RegisterVcnNodeReq)
...
- internal/transport (.CreateService())
...
- internal/transport (.CreateNamespace())
- internal/transport (.ConnectService())
...
- PostgreSQL Database
- internal/clients (.CreateNamespace())
...
- Community 334
- Community 335
- internal/domain (AssembleConfig)
...
- matreshka_common.pb.go
- Tasks
- FromPorts
- NodeStatusFromLastOnline
- CopyToVolumeTaskPayload
...
- autoupgrade.Start() blocks the errgroup
...
- PluginMatrix widget
- CreateSmerd.Request has unimplemented fields (plain config, sidecar, resources)
- Custom.Stop() does nothing
- deleteBatch() is a stub
- DeleteNamespace not implemented
- DeployPage (existing deploy form)
- Deploy watcher never tracks active deployments
...
- Velez PaaS Roadmap
- UI Coding Rules (function declarations, CSS Modules, rem units)
- T26 — ControlPlanePage rebuild
- T27 — DeploymentsPage rebuild
- T28 — VCNPage rebuild
- T29 — SearchPage
- T31 — Wire mock pages to real API (frontend)
- T33 — ServiceInfoPage full redesign
- T34 — ObservabilityLinksPanel widget
- T35 — Environment tab switcher + tags strip
- Velez UI Redesign Roadmap
...
- Toaster component
- autoupgrade.go select falls through instead of returning on stopC
- deployment_specifications.name UNIQUE constraint causes duplicate-key error on second deploy
- tests/helper_environment.go:38 missing local_state import (tests don't compile)
- Suite 3 — Service and deployment lifecycle (Mode B)
- Suite 5 — Multi-node with PG (Mode B)
- golangci-lint configuration
- Nil pointer dereference in healthcheck when containerId is nil
- Healthcheck silently passes when container never starts
- ListDeployments.Response is structurally empty (proto)
...
- Matreshka Coupling (synchronous config fetch in launch pipeline)
- No deployment event log (status transitions overwritten)
...
- Rollback uses context.Background() ignoring deadline
- MainLayout component
- Router.tsx
- Routes.ts route enum
- Routes.Search route constant
- SCHEDULED_UPGRADE case is empty — upgrades silently do nothing
...
- SetupVcn silently disables itself on any failure
- Sidebar active nav highlight bug (activeNodeId vs activeNav)
- Sidebar widget
- SmerdMetaSection component
- sqlc.yaml query generation config
- syncRunningBatch() is a stub
- T38 - PluginManageDialog scaffold (referenced)
- Task file template (000)
- UpgradeDeploy() is a stub that returns success
- UpgradeSmerd pipeline has no self-upgrade guard
- UpgradeSmerd.Response is empty
- NetworkTopologyMap widget
- VCNPeerRow (VCNPeerData type)
- VCNPeerTable widget
- VervClosedNetworkPage component
- VervAppService response is name+id only
- Worker Stop() is a no-op (deploy watcher)
- wrapPgErr is a no-op
...
- create_service.go
- .WithTx
- file_matreshka_common_proto_rawDescGZIP
- StopService_Response
- CreateService
- CreateSmerd
...
1. `T` - 180 edges
2. `NodeClients` - 58 edges
3. `newFakeDocker()` - 49 edges
4. `file_service_api_proto_rawDescGZIP()` - 47 edges
5. `newFakeNodeClients()` - 35 edges
6. `useToaster` - 34 edges
7. `file_velez_api_proto_rawDescGZIP()` - 32 edges
8. `file_matreshka_api_proto_rawDescGZIP()` - 31 edges
9. `file_control_plane_api_proto_rawDescGZIP()` - 30 edges
10. `CreateSmerd_Request` - 30 edges
...
- `VpnSuite` --references--> `ControlPlaneAPIServer`  [EXTRACTED]
  tests/e2e/suite_vpn_test.go → internal/api/server/velez_api/control_plane_api_grpc.pb.go

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **Velez Configuration Environment Variable Schema** — config_config_config_config_config_config_config_config_config_config_config_config_config_configyaml, config_configtemplate, docs_configuration [INFERRED 0.90]
...
## Communities (759 total, 387 thin omitted)

### Community 0 - "internal/api (service_api.pb.gw.go)"
Cohesion: 0.06
Nodes (82): ClientConnInterface, Context, CreateDeploy_Request, CreateDeploy_Response, CreateService_Request, CreateService_Response, GetService_Request, GetService_Response (+74 more)
...
Cohesion: 0.04
Nodes (19): file_matreshka_api_proto_init(), file_matreshka_api_proto_rawDescGZIP(), Message, MessageState, SizeCache, UnknownFields, init(), CreateConfig (+11 more)
...
Cohesion: 0.27
Nodes (5): Smerd, storeToLocalStorage(), useSettings(), SmerdCard(), SmerdsWidget()

### Community 6 - "pkg/web (ServiceCard.tsx)"
Cohesion: 0.07
Nodes (41): router, Arguments, AppCard(), AppCardProps, EnvChip(), EnvChipProps, FreezeChip(), IncidentChip() (+33 more)
...
Cohesion: 0.07
Nodes (39): ControlPlaneAPI, EnableStatefullCluster, ListEnvironmentsResponse, VervPlugin, VervPluginState, VervPluginType, Dialog(), DialogManager (+31 more)

### Community 9 - "pkg/web (ServiceInfoPage.tsx)"
Cohesion: 0.06
Nodes (44): CreateSmerdRequest, Toast, Toaster, useToaster, SkeletonDeploymentHistory(), LoaderWithErrorProps, LoaderWrapper(), DeployMenu() (+36 more)

### Community 10 - "docs/review (SCHEDULED_UPGRADE case is empty — upgrades silently do nothing)"
Cohesion: 0.05
Nodes (39): deployments table missing operational fields (replica_count, rollback_target, owner_node), Architecture & Design Concerns, Cluster State Is Node-Hardcoded, Container Runtime Is Not Abstracted, Deployment History Is Overwritten, Matreshka Coupling, Network Model Assumes Docker Bridge, No Pod or Grouping Concept (+31 more)

### Community 11 - "internal/clients (create.go)"
Cohesion: 0.09
Nodes (24): EnableStatefullTaskPayload, NewEnableStatefullHandler(), enableStatefullTask(), VelezTask, protoBool(), TestCreatePgUserJob_ConnectionFailure(), TestEnableStatefullHandler_Action(), TestEnableStatefullHandler_NewContext() (+16 more)
...
Cohesion: 0.11
Nodes (8): Pointer, NewContainer(), Mutex, NewPortManager(), Container, PortManager, portManagerImpl, GetSharedPortManager()

### Community 14 - "internal/storage (storage.go)"
Cohesion: 0.04
Nodes (21): ClusterStateManager, Storage, Pointer, NewContainer(), Context, Pointer, Storage, Querier (+13 more)

### Community 15 - "internal/pipelines (fetch_by_api.go)"
Cohesion: 0.07
Nodes (27): Absent, BaseEnableHeadscaleServer, BaseEnablePluginRequest, ConnectSlave, ConnectSlaveRequest, ConnectSlaveResponse, EnableHeadscaleServer, EnableHeadscaleServerDeployHeadscaleConfig (+19 more)

### Community 16 - "internal/transport (Pipeliner)"
Cohesion: 0.17
Nodes (14): Context, Docker, Volume, mountedFolders(), TestMountedFolders_DedupPreservesFirstSeenOrder(), TestMountedFolders_Empty(), writeFileToContainer(), copyAPI (+6 more)
...
Cohesion: 0.11
Nodes (17): CreateServiceRequest, useCredentialsStore, Button(), ButtonProps, NewServicePage(), HeaderProps, CreateService(), InitServiceWidget() (+9 more)

### Community 20 - "pkg/web (VervClosedNetworkPage.tsx)"
Cohesion: 0.05
Nodes (41): ListNodesResponse, NodeBaseInfo, NodeStatus, Badge(), BadgeProps, IconButton(), IconButtonProps, SectionLabel() (+33 more)

### Community 21 - "pkg/web (Router.tsx)"
Cohesion: 0.09
Nodes (18): MainLayout(), NAV_TO_ROUTE, NavId, ROUTE_TO_NAV, TOOL_TO_ROUTE, ToolId, ControlPlanePage(), ListNodesQuery() (+10 more)
...
Cohesion: 0.03
Nodes (56): AboutService, Absent, BaseCreateDeployRequest, BaseGetServiceResponse, BoundResource, CreateDeploy, CreateDeployRequest, CreateDeployRequestUpgrade (+48 more)

### Community 24 - "internal/clients (cluster.go)"
Cohesion: 0.10
Nodes (15): ClusterStateManagerContainer, Configurator, ServiceDiscovery, clusterClients, MakoshBeAPIClient, MatreshkaBeAPIClient, Context, Storage (+7 more)
...
Cohesion: 0.12
Nodes (16): ConfigFormat, Connection, Container, ContainerHardware, ContainerHealthcheck, ContainerSettings, Image, MatreshkaConfigSpec (+8 more)

### Community 27 - "internal/app (custom.go)"
Cohesion: 0.09
Nodes (18): Custom, Impl, App, Context, Listener, New(), App, App (+10 more)
...
Cohesion: 0.08
Nodes (9): MessageState, SizeCache, UnknownFields, ConnectSlave_Request, ConnectSlave_Response, EnablePlugin, InitMaster_Response, ListPlugins (+1 more)
...
Cohesion: 0.08
Nodes (23): Closable, EnableStatefullClusterRequest, ListNodesReq, NodeBaseInfo, NodesList, StateClusterDefinition, CreateRequest, NodeBaseInfo (+15 more)

### Community 32 - "internal/service (services.go)"
Cohesion: 0.05
Nodes (39): 1.1 Create and list a simple container, 1.2 Create container with healthcheck, 1.3 Drop a running container, 1.4 Declarative deploy idempotency, 1.5 Duplicate name rejected without declarative flag, 1.6 Upgrade container image, 1.7 Self-upgrade guard, 2.1 Named volume persists across container recreation (+31 more)

### Community 33 - "internal/service (list_test.go)"
Cohesion: 0.23
Nodes (8): Connection, Context, DropSmerd_Request, DropSmerd_Response, ListSmerds_Request, ListSmerds_Response, Smerd, testContainerService
...
Cohesion: 0.09
Nodes (20): AppInfo, Config, EnvironmentConfig, ServersConfig, MakoshBeAPIClient, NewClient(), NewServiceDiscovery(), NewSecurityManager() (+12 more)

### Community 38 - "internal/api (VervAppService)"
Cohesion: 0.13
Nodes (5): EnumDescriptor, EnumNumber, EnumType, DeploymentStatus, NodeType

### Community 39 - "pkg/web (services.ts)"
Cohesion: 0.10
Nodes (18): VelezTask, Context, Mutex, Queries, Tx, VelezTask, newTasksStorage(), NullRawMessage (+10 more)
...
Cohesion: 0.17
Nodes (11): VervClosedNetworkClient, ConnectServiceToVcn, ConnectServiceToVpn(), pipeliner, ServiceDiscovery, GetClientKey(), Context, Context (+3 more)
...
Cohesion: 0.09
Nodes (8): Container_Hardware, Container_Healthcheck, Container_Settings, MatreshkaConfigSpec, RestartPolicy, isCreateSmerd_Request_Config, CreateSmerd_Request, CreateSmerd_Request_Verv
...
Cohesion: 0.15
Nodes (11): Credentials, ls, InitReq, Settings, SettingsBase, Errors, GrpcError, parseGrpcError() (+3 more)

### Community 49 - "internal/pipelines (do_copy_to_volume.go)"
Cohesion: 0.22
Nodes (8): CopyToVolumeRequest, pipeliner, NewCopyToVolumeRunner(), Runner, Exec(), Context, Docker, execStep

### Community 50 - "internal/clients (NodeClients)"
Cohesion: 0.09
Nodes (18): Container, Docker, Context, Setup(), Context, SetupMasterPg(), Client, Context (+10 more)
...
Cohesion: 0.07
Nodes (72): CopyToVolumeTaskPayload, T, Time, NewAssembleConfigHandler(), TestAssembleConfigHandler_Action(), TestAssembleConfigHandler_NewContext(), TestClassifyImage(), TestCreateScratchContainerJob_ContainerCreateError() (+64 more)
...
### Community 56 - "internal/api (Node)"
Cohesion: 0.25
Nodes (6): AssembleConfig, InspectResponse, IsMatreshkaImage(), fromMatreshkaYamlToEvon(), fromYamlToEvon(), pipeliner
...
Cohesion: 0.16
Nodes (11): countTotal(), fromStorageToDomainService(), Context, Querier, SelectBuilder, Service, ServiceBaseInfo, VelezService (+3 more)
...
Cohesion: 0.07
Nodes (26): Architecture: Two Viable Approaches, Backend PaaS (docs/paas/milestones/) — M1–M5, Current State Assessment, For Claude Code sub-agent mode (Option A), Integration with Velez Tasks, Option A — Claude Code as Orchestrator + Anthropic Sub-agents (Recommended for complex tasks), Option B — Shell Factory with Direct Ollama API Calls (Recommended for bulk/template generation), Optional (+18 more)

### Community 65 - "pkg/web (HomePage.tsx)"
Cohesion: 0.09
Nodes (13): ServiceBaseInfo, SmerdStatus, SkeletonLoader(), SkeletonLoaderProps, SkeletonNodeRow(), SkeletonServiceCard(), SkeletonSmerdRow(), formatLastDeployed() (+5 more)

### Community 67 - "internal/api (ConfigInfo)"
Cohesion: 0.08
Nodes (20): ContainerManager, Configurator, Container, Docker, Service, handleConfigurationSubscription(), Context, Impl (+12 more)
...
Cohesion: 0.07
Nodes (27): New(), NewTxManager(), Queries, newDeploymentsStorage(), Queries, newJobsStorage(), calculatePluginState(), Context (+19 more)

### Community 70 - "pkg/web (verv_closed_network.pb.ts)"
Cohesion: 0.10
Nodes (19): ConnectService, ConnectServiceRequest, ConnectServiceResponse, ConnectUser, ConnectUserResponse, CreateVcnNamespace, CreateVcnNamespaceRequest, CreateVcnNamespaceResponse (+11 more)
...
Cohesion: 0.19
Nodes (10): AboutService, CreateDeployReq, Service, ServiceBaseInfo, UpgradeDeployReq, DeploymentStatus, ServiceBaseInfo, Time (+2 more)

### Community 73 - "tests/e2e (NewEnvironment())"
Cohesion: 0.09
Nodes (29): ControlPlaneSuite, LifecycleSuite, StateOpt, TestEnvironment, TestEnvOpt, App, ClientConn, Context (+21 more)
...
Cohesion: 0.16
Nodes (10): Client, newClient(), getKeyFromMatreshkaContainerEnv(), APIClient, Context, initKey(), Client, Context (+2 more)
...
Cohesion: 0.10
Nodes (17): envContainer, pgStorage, staticStorage, Docker, Storage, VervService, New(), Context (+9 more)
...
Cohesion: 0.12
Nodes (4): MessageState, CreateDeploy_Response, GetServiceEnvironments, StopService
...
Cohesion: 0.09
Nodes (4): Message, CreateService_Response, GetServiceGraph, GetServiceResources

### Community 94 - "pkg/web (Input.tsx)"
Cohesion: 0.20
Nodes (8): CreateServiceReq, pipeliner, ServiceBaseInfo, Context, ServiceBaseInfo, Storage, UpsertServiceState(), upsertServiceState

### Community 98 - "internal/service (configurator.go)"
Cohesion: 0.12
Nodes (11): ConfigurationPatch, APIClient, Configurator, Context, Docker, MatreshkaBeAPI_SubscribeOnChangesClient, MatreshkaBeAPIClient, New() (+3 more)

### Community 99 - "internal/api (Message)"
Cohesion: 0.12
Nodes (3): Message, ConnectSlave, InitMaster_Request

### Community 101 - "internal/api (PatchConfig_Request)"
Cohesion: 0.24
Nodes (9): ListServicesReq, ServiceList, Paging, Service, TestEnrichServiceWithSmerdData(), TestServicePreservesLastDeployedAt(), TestServiceWithoutSmerd(), Optional (+1 more)

### Community 103 - "internal/api (SaveConfig_Request)"
Cohesion: 0.07
Nodes (19): CopyToContainerOptions, APIClient, Context, CreateResponse, ExecOptions, HostConfig, InspectResponse, ListSmerds_Request (+11 more)
...
Cohesion: 0.07
Nodes (26): Absent, AssembleConfig, AssembleConfigRequest, AssembleConfigResponse, BaseCreateSmerdRequest, BreakConnections, BreakConnectionsRequest, BreakConnectionsResponse (+18 more)

### Community 109 - "internal/api (Sort_Type)"
Cohesion: 0.10
Nodes (6): EnumDescriptor, EnumNumber, EnumType, ConfigType, Format, Sort_Type
...
Cohesion: 0.15
Nodes (6): file_velez_api_proto_init(), PlainConfigSpec, init(), BreakConnections, CreateSmerd_Request_Plain, isCreateSmerd_Request_Config
...
Cohesion: 0.17
Nodes (16): DeploymentInfo, DeploymentStatus, DeploymentHistory(), DeploymentHistoryProps, DeployRow(), DeployRowProps, formatTimestamp(), getStatusClass() (+8 more)
...
Cohesion: 0.14
Nodes (12): AppConfig, ConfigMeta, ConfigFormat, ConfigTypePrefix, Configurator, Context, Configurator, Context (+4 more)
...
Cohesion: 0.11
Nodes (13): runner, GetLoginServerUrl(), Context, ValidateServiceName(), Context, SingleFunc(), CheckUpgradeIsAvailable(), Context (+5 more)

### Community 134 - "internal/storage (New())"
Cohesion: 0.13
Nodes (12): Context, newNodesStorage(), BoundResource, Context, Docker, newServiceResourcesStorage(), Docker, Storage (+4 more)
...
Cohesion: 0.12
Nodes (20): EnvCard(), EnvCardProps, ServiceEnvironment, ServiceGraphNode, ServiceResource, ListEnvironmentsQuery(), useGetServiceGraphQuery(), useGetServiceResourcesQuery() (+12 more)
...
Cohesion: 0.10
Nodes (7): file_service_api_proto_init(), CreateDeploy_Request_Upgrade, init(), CreateDeploy_Request, GetService_Response_VervService, isCreateDeploy_Request_Specification, VervAppService

### Community 144 - "internal/api (file_velez_api_proto_rawDescGZIP())"
Cohesion: 0.18
Nodes (3): SizeCache, AssembleConfig, BreakConnections_Response
...
Cohesion: 0.10
Nodes (39): ConnectServiceToVpnTaskPayload, Checkpoint(), Context, TestCheckpoint_FailFastWhenFailed(), TestCheckpoint_RunsAndPersistsContextOnSuccess(), TestCheckpoint_SkipsWhenDone(), connectServiceToVpnTask(), VelezTask (+31 more)
...
Cohesion: 0.25
Nodes (9): createPgUserStep, WithExposedPort(), WithInstanceName(), WithPassword(), WithPort(), pipeliner, CreatePgUserForNode(), Context (+1 more)

### Community 159 - "tests/e2e (suite_hello_world_cluster_test.go)"
Cohesion: 0.05
Nodes (25): AssembleConfigJobSuite, AssembleConfigSuite, VpnSuite, Context, runner[T], addTestLabels(), AssertSmerds(), CreateSmerd_Request (+17 more)

### Community 160 - "internal/api (Patch)"
Cohesion: 0.36
...
Cohesion: 0.14
Nodes (24): NewConnectServiceToVpnHandler(), proto(), TestAddMakoshRecordJob_Success(), TestAddMakoshRecordJob_UpsertError(), TestConnectServiceToVpnHandler_Action(), TestConnectServiceToVpnHandler_BuildJobs_NamesAndOrder(), TestConnectServiceToVpnHandler_NewContext(), TestGetClientKeyJob_ExistingKeyFound() (+16 more)
...
Cohesion: 0.08
Nodes (16): IssueClientKey, RegisterVcnNodeReq, VcnNamespace, Context, Client, Context, Client, Context (+8 more)
...
Cohesion: 0.10
Nodes (13): InputProps, StyleProps, KeyValueProps, PlainMapProps, Search(), SearchParam, SearchProps, InputSearchProps (+5 more)
...
Cohesion: 0.12
Nodes (8): file_tasks_proto_init(), MessageState, SizeCache, UnknownFields, init(), CreateServiceTaskPayload, WatchTask, WatchTask_Request
...
Cohesion: 0.19
Nodes (14): ClusterClients, Pipeliner, NewPipeliner(), Context, New(), Impl, New(), APIClient (+6 more)

### Community 189 - "internal/patterns (headscale.go)"
Cohesion: 0.15
Nodes (11): GetVcnAuthKeyReq, ListVpnNamespaces, SetupHeadscaleRequest, SetupHeadscaleResponse, VcnAuthKey, Context, Client, BasicConfig() (+3 more)

### Community 190 - "TestEnvironment"
Cohesion: 0.23
Nodes (9): Code, fetchConfigStep, isCreateSmerd_Request_Config, FetchConfig(), Context, InspectResponse, MatreshkaConfigSpec, PlainConfigSpec (+1 more)
...
Cohesion: 0.10
Nodes (18): Agent code of conduct, API (Proto definitions in `api/grpc/`), Architecture, Building, Code Generation, Code Style, Commands, Configuration (+10 more)

### Community 197 - "Paging"
Cohesion: 0.27
Nodes (9): getConfigFromContainerStep, ConfigMount, isPostgresByImageTags(), fillMeta(), GetConfigFromContainerStep(), APIClient, Context, InspectResponse (+1 more)
...
Cohesion: 0.19
Nodes (7): ActionButton(), ActionButtonProps, VolumesWidgetProps, ValuesPair(), VolumeMappingProps, Bind, BindingsWidgetProps

### Community 215 - "internal/api (Message)"
Cohesion: 0.13
Nodes (13): PluginBaseInfo, DeploymentStatus, VervPlugin_State, Context, Docker, newPluginsStorage(), Context, NullInt64 (+5 more)
...
Cohesion: 0.22
Nodes (14): SmerdPage(), GetInitReq(), InitReq, keyPath(), pathPrefixPath(), StoreApiKey(), StorePathPrefix(), FetchSmerd() (+6 more)

### Community 236 - "internal/domain (node.go)"
Cohesion: 0.16
Nodes (12): APIClient, Context, Docker, NewCreateSmerdHandler(), containerIdAccessor, createContainerJob, createSmerdHandler, healthcheckJob (+4 more)
...
Cohesion: 0.15
Nodes (14): Context, Duration, VelezTask, NewEngine(), Context, Handler, ServerStreamingServer, VelezTask (+6 more)

### Community 243 - "pkg/web (MockDeployRow.tsx)"
Cohesion: 0.18
Nodes (9): ServerStream, ServerStreamingServer, ServiceRegistrar, RegisterTasksApiServer(), _TasksApi_WatchTask_Handler(), ServiceRegistrar, TasksApiServer, UnimplementedTasksApiServer (+1 more)
...
Cohesion: 0.22
Nodes (6): CreateRequest, TailScaleContainerSidecar(), CheckSidecarExist(), Context, Docker, checkSidecarExist

### Community 248 - "internal/api (GetHardware_Request)"
Cohesion: 0.07
Nodes (20): Context, Container, Context, Duration, Once, Ticker, VelezTask, copyToVolumeHandler (+12 more)

### Community 250 - "internal/api (ListDeployments)"
Cohesion: 0.22
Nodes (3): ConnectUserRequest, VcnApi, ConnectUserToVcn()

### Community 256 - "ListEnvironments_Response"
Cohesion: 0.24
Nodes (8): ContainerState, APIClient, Context, Docker, EndpointSettings, InspectResponse, PauseContainer(), detachContainerFromVervStep

### Community 259 - "internal/api (StopService)"
Cohesion: 0.17
Nodes (5): UnknownFields, fromApiNodes(), Configurator, Context, Node

### Community 261 - "internal/api (UpgradeSmerd)"
Cohesion: 0.18
Nodes (3): MessageState, MakeConnections, UpgradeSmerd

### Community 264 - ".claude/factory (new-task.sh)"
Cohesion: 0.70
Nodes (4): err(), log(), ok(), new-task.sh script

### Community 265 - "fakeJobsStorage"
Cohesion: 0.23
Nodes (9): copyToContainerStep, FileMountPoint, APIClient, Context, ReadFromContainer(), WriteToContainer(), CopyToContainer(), APIClient (+1 more)
...
Cohesion: 0.10
Nodes (12): DropSmerdRequest, DropSmerdResponse, ListSmerdsRequest, ListSmerdsResponse, SearchImagesRequest, SearchImagesResponse, UpgradeSmerdRequest, VelezAPI (+4 more)
...
### Community 282 - "internal/transport (.CreateService())"
Cohesion: 0.40
...
### Community 289 - "internal/transport (.CreateNamespace())"
Cohesion: 0.24
Nodes (7): Create(), APIClient, Context, Docker, HostConfig, NetworkingConfig, createSmerdStep

### Community 290 - "internal/transport (.ConnectService())"
Cohesion: 0.40
Nodes (4): ConnectService_Request, ConnectService_Response, Context, Impl
...
### Community 311 - "internal/clients (.CreateNamespace())"
Cohesion: 0.20
Nodes (9): ToDockerEnv(), FromVolume(), Container_Settings, Volume, ToVolume(), ContainerManager, Context, Smerd (+1 more)
...
Cohesion: 0.36
Nodes (9): FromPorts(), Container_Settings, ToPort(), ToPortProtocol(), ToPortsMapping(), ToPortsSlice(), Port, Port_Protocol (+1 more)

### Community 342 - "prepareVervConfig"
Cohesion: 0.31
...
Cohesion: 0.21
Nodes (16): ClientConnInterface, NewTasksApiClient(), ClientConn, Context, DialOption, Marshaler, Request, ServeMux (+8 more)

### Community 353 - "pkg/web (@tanstack/react-query)"
Cohesion: 0.15
Nodes (9): M3 — Observability, Scope (draft), M4 — Access & Multi-tenancy, Scope (draft), M5 — PaaS Automation, Scope (draft), Milestones, UI Redesign (complete) (+1 more)

### Community 354 - "pkg/web (vite)"
Cohesion: 0.15
Nodes (10): Component, CSS, Props interface, T06 — IconButton, What it looks like, Component, CSS, Props interface (+2 more)

### Community 429 - "Velez UI Redesign — Roadmap"
Cohesion: 0.15
Nodes (13): Backend Milestone 3 — Plugin Service, Coding Rules (must be followed in every task), Milestone 10 — Plugin Enable UX, ~~Milestone 1 — Design Tokens~~ ✓, ~~Milestone 2 — Base Components (`src/components/base/`)~~ ✓, ~~Milestone 3 — Complex Components (`src/components/complex/`)~~ ✓, ~~Milestone 4 — Widgets (`src/widgets/`)~~ ✓, ~~Milestone 5 — Layout / Segments~~ ✓ (+5 more)

### Community 430 - "ValuesMapping.tsx"
Cohesion: 0.12
Nodes (21): Volume, Checkbox(), CheckboxProps, InfoMark(), InfoMarkProps, CreateSmerdReq, fromProto(), fromProtoPort() (+13 more)
...
### Community 432 - "dockerServiceDepsStorage"
Cohesion: 0.11
Nodes (17): NodeType, ServiceDependency, ServiceEnvironment, ServiceGraph, Time, Context, VervService, containerName() (+9 more)

### Community 433 - "matreshka_common.pb.go"
Cohesion: 0.25
...
### Community 435 - "FromPorts"
Cohesion: 0.19
Nodes (8): HealthConfig, FromCommand(), FromHealthcheck(), Container_Healthcheck, FromDockerEnv(), FromRestart(), RestartPolicy, StrSlice
...
### Community 444 - "GetHardware_Request"
Cohesion: 0.18
Nodes (3): UnknownFields, GetHardware, GetHardware_Request

### Community 445 - "WatchTask_Request"
Cohesion: 0.22
Nodes (4): Context, Timestamp, ServerStreamingClient, TaskStatus

### Community 446 - ".enrichServiceWithSmerdData"
Cohesion: 0.28
Nodes (6): Context, ServiceBaseInfo, VervService, mapSmerdStatus(), TestMapSmerdStatus(), Smerd_Status

### Community 447 - "GetService"
Cohesion: 0.12
Nodes (15): ServiceBaseInfo, TestToServiceBaseInfoList(), TestToServiceBaseInfoWithEmptyFields(), TestToServiceBaseInfoWithEnrichedFields(), toServiceBaseInfo(), toServiceBaseInfoList(), Context, GetServiceEnvironments_Request (+7 more)
...
### Community 451 - ".UpgradeSmerd"
Cohesion: 0.28
Nodes (6): pipeliner, DropContainerStep(), Context, Docker, dropContainer, UpgradeSmerd
...
### Community 466 - "fromContainerToRequest"
Cohesion: 0.28
Nodes (5): FromContainerToRequest(), Context, Smerd, NetworkBind, fromContainerToRequest

### Community 467 - "ListServices"
Cohesion: 0.12
Nodes (4): UnknownFields, GetVervonomicon_Response, ListDeployments, ListServices
...
### Community 482 - "DisconnectFromNetworks"
Cohesion: 0.43
...
### Community 486 - "renameContainerStep"
Cohesion: 0.38
...
### Community 496 - "T15 — CodeBlock"
Cohesion: 0.33
Nodes (5): Component, CSS, Props interface, T15 — CodeBlock, What it looks like

### Community 497 - "GetVervonomicon"
Cohesion: 0.40
Nodes (4): AddMakoshRecord(), Context, ServiceDiscovery, addMakoshRecord

### Community 498 - "StoreConfig"
Cohesion: 0.43
Nodes (5): storeConfigStep, ConfigTypePrefix, Configurator, Context, StoreConfig()

### Community 499 - ".ListSmerds"
Cohesion: 0.12
Nodes (4): SizeCache, GetService, RestartService, RestartService_Response
...
### Community 507 - "GetServiceMetrics"
Cohesion: 0.12
Nodes (3): file_service_api_proto_rawDescGZIP(), GetServiceMetrics, GetServiceMetrics_Request

### Community 742 - "Jobs Migration — Open Questions"
Cohesion: 0.33
Nodes (5): ConnectServiceToVpn — resolved 2026-07-19, CopyToVolume (added during implementation), CopyToVolume — resolved 2026-07-19, Cross-cutting (all pipelines), Jobs Migration — Open Questions

### Community 744 - "ConnectToContainer"
Cohesion: 0.62
Nodes (6): Connect(), ConnectToContainer(), getApiAddress(), Context, Docker, Client

### Community 745 - "PrepareImage"
Cohesion: 0.43
Nodes (5): Context, Docker, InspectResponse, PrepareImage(), stepPrepareImage

### Community 746 - "Start"
Cohesion: 0.38
Nodes (4): APIClient, Context, Start(), smerdStart

### Community 748 - ".WithTx"
Cohesion: 0.33
...
### Community 754 - ".ListSmerds"
Cohesion: 0.40
Nodes (4): ContainerManager, Context, ListSmerds_Request, ListSmerds_Response

### Community 755 - "overview.md"
Cohesion: 0.40
Nodes (4): Approach, Exit criteria, M1 — Core Platform, Task groups
...
- **Why does `T` connect `tests/e2e (T)` to `tests/e2e (suite_hello_world_cluster_test.go)`, `pkg/web (VcnApi)`, `internal/pipelines (Step)`, `internal/api (PatchConfig_Request)`, `tests/e2e (NewEnvironment())`, `GetService`, `internal/clients (create.go)`, `internal/clients (PortManager)`, `internal/transport (Pipeliner)`, `pkg/web (ControlPlanePage.tsx)`, `pkg/web (fetch.pb.ts)`, `.enrichServiceWithSmerdData`, `internal/api (ConfigBase)`?**
  _High betweenness centrality (0.155) - this node is a cross-community bridge._
- **Why does `file_velez_common_proto_init()` connect `internal/api (velez_common.pb.go)` to `internal/api (velez_api.pb.go)`, `pkg/web (package.json)`, `internal/api (control_plane_api.pb.go)`, `internal/api (CreateDeploy_Request)`?**
  _High betweenness centrality (0.125) - this node is a cross-community bridge._
- **Why does `NodeClients` connect `internal/clients (NodeClients)` to `ListEnvironments_Response`, `fakeJobsStorage`, `internal/clients (create.go)`, `pkg/web (DeployWidget.tsx)`, `internal/transport (Pipeliner)`, `internal/app (custom.go)`, `pkg/web (VcnApi)`, `internal/transport (.CreateNamespace())`, `internal/config (launch.go)`, `internal/pipelines (create_and_inspect.go)`, `internal/pipelines (ConnectServiceToVpn())`, `internal/pipelines (ConfigurationService)`, `internal/pipelines (LaunchSmerd)`, `internal/pipelines (do_copy_to_volume.go)`, `internal/pipelines (.UpgradeSmerd())`, `tests/e2e (T)`, `internal/domain (vcn.go)`, `.UpgradeSmerd`, `Paging`, `internal/pipelines (fetch_from_container.go)`, `internal/storage (service_dependencies.go)`, `prepareVervConfig`, `internal/api (Message)`, `renameContainerStep`, `ConnectToContainer`, `PrepareImage`, `Start`, `internal/domain (node.go)`, `internal/api (CreateDeploy)`, `internal/api (GetHardware_Request)`?**
  _High betweenness centrality (0.117) - this node is a cross-community bridge._
- **Are the 47 inferred relationships involving `newFakeDocker()` (e.g. with `TestCreateScratchContainerJob_ContainerCreateError()` and `TestCreateScratchContainerJob_Rollback_NoContainerId_NoOp()`) actually correct?**
  _`newFakeDocker()` has 47 INFERRED edges - model-reasoned connections that need verification._
