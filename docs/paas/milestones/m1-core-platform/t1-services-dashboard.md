...
- [x] Last-deployed timestamp — requires API change: `ServiceBaseInfo` must include `lastDeployedAt`
...
- [x] Stop / Restart actions — requires new API endpoints (`StopService`, `RestartService`) — not present in gRPC API
