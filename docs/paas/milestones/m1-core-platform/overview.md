...
**Goal:** a working browser UI where an operator can see what is running, inspect deployments, trigger new deploys,
manage Verv services, and configure the connection to the backend.
...
- [ ] Verv services can be created, viewed, and removed (create + view + delete UI done; backend delete API not yet
  wired)
...
Increment on what already exists in `src/pages/` rather than rewriting. Each task group is a self-contained slice — UI,
API process calls, and model types — so groups can be worked in parallel.
