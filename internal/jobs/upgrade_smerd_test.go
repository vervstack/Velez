...
func TestUpgradeSmerdHandler_BuildJobs_NamesAndOrder(t *testing.T) {
	payload := &velez_api.UpgradeSmerdTaskPayload{
		UpgradeRequest: &velez_api.UpgradeSmerd_Request{Name: "mysvc", Image: "myimg:new"},
	}
	...
	wantNames := []string{
		"check_self_upgrade", "capture_old_container", "prepare_image", "pause_old_container",
		"create_config_fetcher_container", "get_config_from_container", "drop_config_fetcher_container",
		"fetch_config", "prepare_verv_config", "create_final_container", "start_final_container",
		"healthcheck", "rename_old_container", "drop_old_container", "rename_new_container",
	}
...
func TestCheckSelfUpgradeJob_NotInsideContainer_NoOp(t *testing.T) {
	...
	payload := &velez_api.UpgradeSmerdTaskPayload{
		UpgradeRequest: &velez_api.UpgradeSmerd_Request{Name: "mysvc"},
	}
...
func TestCaptureOldContainerJob_Success(t *testing.T) {
	...
	containerService.inspectResp = &velez_api.Smerd{
		...
		Networks: []*velez_api.NetworkBind{
			{NetworkName: "vervnet", Aliases: []string{"myalias", "old123"}},
		},
...
func TestPrepareUpgradeImageJob_PullError(t *testing.T) {
	...
	payload := &velez_api.UpgradeSmerdTaskPayload{
		UpgradeRequest: &velez_api.UpgradeSmerd_Request{Image: "myimg:new"},
	}
...
func TestPrepareUpgradeImageJob_Success(t *testing.T) {
	...
	docker.pullImageResp = image.InspectResponse{
		Config: &dockerspec.DockerOCIImageConfig{
			ImageConfig: ocispec.ImageConfig{Labels: map[string]string{labels.MatreshkaConfigLabel: "true"}},
		},
		RepoTags: []string{"myimg:new"},
	}
	...
	if payload.GetImageLabels()[labels.MatreshkaConfigLabel] != "true" {
...
func TestPauseOldContainerJob_Success(t *testing.T) {
	...
	containerAPI.inspectResp = container.InspectResponse{
		...
		NetworkSettings: &container.NetworkSettings{
			NetworkSettingsBase: container.NetworkSettingsBase{
				Ports: nat.PortMap{"8080/tcp": []nat.PortBinding{{HostPort: "40001"}}},
			},
			Networks: map[string]*network.EndpointSettings{"vervnet": {Aliases: []string{"myalias"}}},
		},
	}

	payload := &velez_api.UpgradeSmerdTaskPayload{OldContainerId: strPtr("old123")}

	j := &pauseOldContainerJob{dockerAPI: containerAPI, portManager: realPortManager(t), ctx: payload}

	err := j.Do(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(containerAPI.pauseCalledWith) != 1 || containerAPI.pauseCalledWith[0] != "old123" {
	...
	if len(j.portsOnHold) != 1 || j.portsOnHold[0] != 40001 {
		t.Errorf("expected port 40001 held, got %v", j.portsOnHold)
	}
	...
	// per-call state), so ContainerInspect still reports "vervnet" present
...
func TestRenamingCreateContainerJob_Success(t *testing.T) {
	docker := newFakeDocker()
	docker.containerCreateResp = container.CreateResponse{ID: "created123"}
	containerAPI := newFakeContainerAPI()
	containerAPI.inspectResp = container.InspectResponse{
		ContainerJSONBase: &container.ContainerJSONBase{ID: "created123"},
	}
	docker.withClient(containerAPI)
	nodeClients := newFakeNodeClients(docker)

	payload := &velez_api.UpgradeSmerdTaskPayload{
		Request: &velez_api.CreateSmerd_Request{Name: "mysvc", Settings: &velez_api.Container_Settings{}},
	}
	...
	if payload.GetRequest().GetName() != "mysvc"+configFetcherContainerSuffix {
		t.Errorf("expected request name renamed, got %q", payload.GetRequest().GetName())
	}
	if payload.GetContainerId() != "created123" {
...
func TestFetchUpgradeConfigJob_RestoresNameAndMergesEnv(t *testing.T) {
	...
	payload := &velez_api.UpgradeSmerdTaskPayload{
		...
		Request: &velez_api.CreateSmerd_Request{
			Name: "mysvc" + configFetcherContainerSuffix,
	...
	if payload.GetRequest().GetName() != "mysvc" {
...
func TestPrepareUpgradeVervConfigJob_Success(t *testing.T) {
	...
	payload := &velez_api.UpgradeSmerdTaskPayload{
		Request: &velez_api.CreateSmerd_Request{
			...
			Settings: &velez_api.Container_Settings{
				Ports:   []*velez_api.Port{{ServicePortNumber: 8080}},
				Network: []*velez_api.NetworkBind{{NetworkName: "vervnet"}},
			},
		},
	...
	if payload.GetRequest().GetLabels()[labels.ComposeGroupLabel] != "mysvc" {
...
func TestRenameContainerJob_EmptyContainerId_Error(t *testing.T) {
	...
	j := &renameContainerJob{ctx: payload, newName: "mysvc"}
...
func TestRenameContainerJob_SuccessAndRollback(t *testing.T) {
	...
	j := &renameContainerJob{dockerAPI: containerAPI, ctx: payload, newName: "mysvc"}

	err := j.Do(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(containerAPI.renameCalledWith) != 1 || containerAPI.renameCalledWith[0].newName != "mysvc" {
...
func upgradeSmerdTask(t *testing.T, tasksStorage *fakeTasksStorage, entityID string, payload *velez_api.UpgradeSmerdTaskPayload) tasks_queries.VelezTask {
	t.Helper()
...
func TestUpgradeSmerdHandler_HappyPath_EndToEnd(t *testing.T) {
	...
	task := upgradeSmerdTask(t, tasksStorage, "mysvc", payload)
	...
	docker.containerCreateResp = container.CreateResponse{ID: "cont-fixture"}
	docker.withClient(containerAPI)
	...
	containerService.inspectResp = &velez_api.Smerd{
		...
		ImageName: "myimg:old",
		Ports:     []*velez_api.Port{{ServicePortNumber: 8080}},
		Networks:  []*velez_api.NetworkBind{{NetworkName: "vervnet", Aliases: []string{"myalias"}}},
		Env:       map[string]string{"FOO": "bar"},
		Labels:    map[string]string{"team": "core"},
	}
	...
	if finishedPayload.GetOldContainerId() != "old123" {
	...
	// the real Docker container gets renamed back to "mysvc" (rename_new_container),
	// matching do_smerd_upgrade.go's own newLaunch, which is never touched
	// again after its last SingleFunc step either.
	if finishedPayload.GetRequest().GetName() != "mysvc"+newContainerSuffix {
...
func TestUpgradeSmerdHandler_FailurePath_NetworkCreateFails(t *testing.T) {
	...
	// connectToNetwork's already-connected check sees "vervnet" as still
