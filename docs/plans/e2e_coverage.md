...
| `Test_Vpn` (`suite_vpn_test.go`) | yes | Phase 4. Real headscale (shared per-binary fixture `shared_headscale.go`). `Test_NamespaceCrud`: Create → id, List contains, duplicate → `AlreadyExists`, Delete → gone, all through transport → vcn client → real headscale HTTP API + real sqlite. `Test_ConnectService_LaunchesSidecar`: `ConnectService` RPC drives `connect_service_to_vpn` to DONE — real namespace + real pre-auth-key on the fixture headscale — and a real, running `<svc>-ts-sidecar` container. **Not** covered: actual tailnet join — `getLoginServerURLJob` hardcodes `https://vcn.redsock.ru` (Trello #126 follow-up). |
...
| Headscale `ConnectService` (tailscale sidecar) | covered (sidecar up) | `Test_ConnectService_LaunchesSidecar` — real namespace + pre-auth key + running sidecar container; tailnet join itself not asserted (Trello #126 follow-up) |
| Headscale namespace CRUD (Create/List/Delete) | covered | `Test_NamespaceCrud` against real headscale + sqlite |
...
## 4. Phase 4: real headscale (Trello #126) — DONE (login-server piece deferred)

Landed as the achievable slice the user approved: a shared per-test-binary headscale container
in the DinD harness (`tests/e2e/shared_headscale.go`, same `sync.Once` shape as
`shared_matreshka.go`), `dindHeadscalePort = 30021` carved out of the PortManager band and
published by the DinD daemon, `headscale/headscale:0.27.2-rc.1` + `tailscale/tailscale:v1.90.8`
added to `dindSeedImages`. The fixture self-provisions a minimal config (embedded DERP so
headscale boots, sqlite), execs `headscale apikey create`, and hands the app a
bootstrap-host-reachable URL + key. `WithStateVcnEnabled(serverURL, apiKey)` now sets **both**
`Network.Headscale.ServerUrl` and `.Key`, so `verv_closed_network.SetupVcn` takes the
`headscale.Connect(url, key)` branch — the app never launches its own headscale, **no product
change**. `Test_Vpn` un-skipped: `Test_NamespaceCrud` + `Test_ConnectService_LaunchesSidecar`.

**Deferred to a separate task (Trello #126 follow-up):** verifying an actual tailnet join.
`internal/jobs/connect_service_to_vpn.go`'s `getLoginServerURLJob.Do()` hardcodes
`j.ctx.SetLoginServerUrl("https://vcn.redsock.ru")`, so the tailscale sidecar always dials that
public URL regardless of which headscale is running — a hermetic join can't be asserted until
that URL is sourced from state/config. The e2e asserts the sidecar **container** is created and
running, not that it joined. The user also plans to rework how the headscale container spins up,
after which the fixture and `WithStateVcnEnabled` can be revisited.

The user-provided compose/config below are kept for reference; the fixture's own config is a
trimmed variant (no `policy.mode: file` / `acl.json`, DERP `region_id: 999`, `server_url` a
placeholder — the API surface exercised doesn't depend on it).
...
**Limitation discovered mid-flight — RESOLVED in the C follow-up commit `[Jobs] Fix: resolve
storage backend live ...` (see §5). `TODO(#125)` is cleared; `Test_ServiceDeploymentLifecycle`
now drives the full watcher path to RUNNING and through `CreateDeploy(Upgrade)`.** Original
description retained below for context:**
...
| 4 | `[Tests] E2E: real headscale VPN fixture (un-skip Test_Vpn)` | **No product change.** New `tests/e2e/shared_headscale.go`: process-wide `sync.Once` fixture (mirrors `shared_matreshka.go`) that runs one `headscale/headscale:0.27.2-rc.1` container in the DinD, `CopyToContainer`s a trimmed self-provisioned config (embedded DERP so headscale boots, sqlite, no `policy` file), execs `headscale apikey create`, and exposes a bootstrap-host-reachable `apiURL` (`sharedDind.Addr`) + `apiKey`. `tests/e2e/dind_ports.go`: `dindHeadscalePort = 30021` carved out of the PortManager band and appended to `dindPublishPorts()`; `headscale/headscale:0.27.2-rc.1` + `tailscale/tailscale:v1.90.8` added to `dindSeedImages`. `tests/e2e/main_test.go`: `sharedHeadscale.stop()` after `m.Run()` (nil-safe). `tests/e2e/helper_environment.go`: `WithStateVcnEnabled(serverURL, apiKey string)` now sets **both** `Network.Headscale.ServerUrl` and `.Key`, so `verv_closed_network.SetupVcn` takes the `headscale.Connect(url, key)` branch — the app never launches its own headscale. `tests/e2e/suite_vpn_test.go` un-skipped and rewritten: `Test_NamespaceCrud` (Create → id, List contains, duplicate → `AlreadyExists`, Delete → gone) and `Test_ConnectService_LaunchesSidecar` (`ConnectService` RPC → `connect_service_to_vpn` task DONE: real namespace + real pre-auth key on the fixture headscale + a real running `<svc>-ts-sidecar` container). **Deferred (Trello #126 follow-up):** actual tailnet join — `getLoginServerURLJob` hardcodes `https://vcn.redsock.ru`, so the sidecar can't be verified as joined in a hermetic DinD; only the container is asserted. `go build ./...` clean; `make lint` 0 issues; `go test -run Test_Vpn ./tests/e2e/...` green (31s); `make test-e2e` green (`ok go.vervstack.ru/Velez/tests/e2e 130.317s`); `graphify update .` run (kept OUT of the commit). |
